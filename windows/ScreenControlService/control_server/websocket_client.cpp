...
void WebSocketClient::log(const std::string& message)
{
    ...
    Logger::getInstance().info(std::wstring(fullMsg.begin(), fullMsg.end()));
}
...
bool WebSocketClient::connect(const DebugConfig& config)
{
    ...
    // Connect
    if (::connect(m_socket, addrs->ai_addr, static_cast<int>(addrs->ai_addrlen)) == SOCKET_ERROR)
    {
    ...
    int written = m_useSSL ? sslWrite(reqStr.c_str(), static_cast<int>(reqStr.size()))
                           : send(m_socket, reqStr.c_str(), static_cast<int>(reqStr.size()), 0);
    ...
    int bytesRead = m_useSSL ? sslRead(buffer, sizeof(buffer) - 1)
                             : recv(m_socket, buffer, sizeof(buffer) - 1, 0);
...
void WebSocketClient::receiveLoop()
{
    ...
    while (m_running && m_connected)
    {
        int bytesRead = m_useSSL ? sslRead(reinterpret_cast<char*>(buffer.data()), static_cast<int>(buffer.size()))
                                  : recv(m_socket, reinterpret_cast<char*>(buffer.data()), static_cast<int>(buffer.size()), 0);
...
bool WebSocketClient::sslConnect(const std::string& host, int port)
{
    // Simplified SSL - for production use OpenSSL or full Schannel implementation
    // For now, we'll log a warning and proceed without SSL for testing
    ...
    while (status == SEC_I_CONTINUE_NEEDED || status == SEC_E_INCOMPLETE_MESSAGE)
    {
        ...
        int bytesRead = recv(m_socket, recvBuffer, sizeof(recvBuffer), 0);
        if (bytesRead <= 0)
        {
        ...
        if (outBuffers[0].cbBuffer > 0 && outBuffers[0].pvBuffer != nullptr)
        {
            send(m_socket, static_cast<char*>(outBuffers[0].pvBuffer), outBuffers[0].cbBuffer, 0);
            FreeContextBuffer(outBuffers[0].pvBuffer);
        }
    }
...
{
    if (!m_sslHandle) return recv(m_socket, buffer, length, 0);

    CtxtHandle* pCtxtHandle = static_cast<CtxtHandle*>(m_sslHandle);

    // Read encrypted data
    char encryptedBuffer[16384];
    int bytesRead = recv(m_socket, encryptedBuffer, sizeof(encryptedBuffer), 0);
...
{
    if (!m_sslHandle) return send(m_socket, data, length, 0);
    ...
    return send(m_socket, encryptedBuffer.data(), totalSize, 0) > 0 ? length : -1;
}
...
void WebSocketClient::sslDisconnect()
{
    ...
    if (m_sslContext)
    {
        CredHandle* pCredHandle = static_cast<CredHandle*>(m_sslContext);
        FreeCredentialsHandle(pCredHandle);
        delete pCredHandle;
        m_sslContext = nullptr;
    }
