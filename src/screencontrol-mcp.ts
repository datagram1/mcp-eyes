      ...
      try {
        ...
        switch (name) {
          ...
          case 'getUIElements':
            ...
            const richElements = allUIElements.map((el: any, i: number) => ({
              index: el.index ?? i,
              type: el.type,
              role: el.role,
          ...
          case 'wait':
            const waitMs = (args?.milliseconds as number) || 1000;
...
${args?.includeScreenshot === true && result.screenshot ? '\n[Screenshot included]' : 'Tip: Set includeScreenshot: true to see the page'}`;
          ...
          case 'browser_fillWithFallback': {
            ...
            const browser = (args?.browser || 'firefox') as string;
          ...
          case 'browser_fillFormNative': {
            ...
            const fields = args?.fields as Array<{ label?: string; selector?: string; value: string }> || [];
            const buttons = args?.buttons as Array<{ label?: string; selector?: string }> || [];
            const shouldSubmit = args?.submit === true;
            const submitSelector = args?.submitSelector as string | undefined;
            const browser = (args?.browser || 'firefox') as string;
            const tabId = args?.tabId;
            ...
            try {
              ...
              const inputElements = elements.filter((el: any) =>
                el.type?.includes('input') || el.type === 'textarea'
              );

              // Step 3: Match requested fields to page elements
              const fieldMappings: Array<{
                field: { label?: string; selector?: string; value: string };
                element: any;
                index: number;
              }> = [];
              ...
              for (const field of fields) {
                ...
                if (field.selector) {
                  // Find by selector
                  matchedElement = elements.find((el: any) => el.selector === field.selector);
                  matchIndex = inputElements.findIndex((el: any) => el.selector === field.selector);
                } else if (field.label) {
                  // Find by label (fuzzy match)
                  const labelLower = field.label.toLowerCase();
                  matchedElement = inputElements.find((el: any) => {
              ...
              for (const { field, element } of fieldMappings) {
                const currentEl = verifyElements.find((el: any) =>
                  el.selector === element.selector || el.id === element.id
                );
          ...
          case 'browser_clickMultiple':
            ...
            return {
              content: [{
                type: 'text',
                text: `Clicked ${result.clickedCount}/${result.totalCount} elements:\n${result.results.map((r: any, i: number) => `  ${i + 1}. ${r.selector}: ${r.success ? 'OK' : r.error}`).join('\n')}`,
              }],
            };
          ...
          case 'browser_getFormStructure':
            ...
            const questionsText = (result.questions || []).map((q: any, i: number) => {
              let questionOutput = `${i + 1}. ${q.text}`;
              if (q.type === 'yes_no') {
                questionOutput += `\n   Yes: ${q.yesSelector}\n   No: ${q.noSelector}`;
              } else if (q.type === 'multiple_choice') {
                questionOutput += `\n   Options:\n${q.options.map((o: any) => `     - "${o.text}": ${o.selector}`).join('\n')}`;
          ...
          case 'browser_answerQuestions':
            ...
            return {
              content: [{
                type: 'text',
                text: `Answered ${result.answeredCount}/${result.totalQuestions} questions:\n${result.results.map((r: any) => `  • "${r.question}": ${r.answer} ${r.success ? '✓' : '✗ ' + r.error}`).join('\n')}`,
              }],
            };
          ...
          case 'browser_listInteractiveElements':
            ...
            if (Array.isArray(interactiveEls)) {
              const formatted = interactiveEls.map((el: any, i: number) => {
                const selectors = el.alternativeSelectors || [];
                const selectorList = selectors.slice(0, 3).map((s: any) => `    ${s.type}: ${s.selector}`).join('\n');
          ...
          case 'browser_clickElementWithDebug':
            ...
            if (result.candidates?.length) {
              debugText += `  Candidates:\n${result.candidates.map((c: any) => `    - ${c.selector}: "${c.text}"`).join('\n')}\n`;
            }
          ...
          case 'browser_getDropdownOptions':
            ...
            const optionsList = opts.map((opt: any, i: number) => {
          ...
          case 'browser_openDropdownNative': {
            ...
            const waitMs = (args?.waitMs as number) || 500;
            const browser = (args?.browser || 'firefox') as string;
            ...
            try {
              ...
              const nativeOptsList = nativeOpts.map((opt: any, i: number) => {
                const coords = opt.screenCoordinates?.normalized;
                return `[${i}] "${opt.text}"\n` +
                       `    selector: ${opt.selector}\n` +
                       `    coords: (${coords?.centerX?.toFixed(3) || '?'}, ${coords?.centerY?.toFixed(3) || '?'})`;
      ...
      switch (name) {
        ...
        case 'fs_write':
          return {
            content: [{
              type: 'text',
              text: `Wrote ${result.bytes_written} bytes to ${path.basename(result.path)}`,
            }],
          };
        ...
        case 'fs_delete':
          return {
            content: [{
              type: 'text',
              text: result.deleted ? `Deleted ${path.basename(result.path)}` : `Failed to delete ${path.basename(result.path)}`,
            }],
          };
        ...
        case 'fs_move':
          return {
            content: [{
              type: 'text',
              text: result.moved ? `Moved ${path.basename(result.from)} to ${path.basename(result.to)}` : `Failed to move`,
            }],
          };
        ...
        case 'fs_patch':
          return {
            content: [{
              type: 'text',
              text: `Applied ${result.operations_applied} operations to ${path.basename(result.path)}${result.preview ? '\n\nPreview:\n' + result.preview.map((p: any) => `${p.operation}: ${p.changed ? 'changed' : 'no change'}`).join('\n') : ''}`,
            }],
          };
