...
export async function GET(request: NextRequest) {
  ...
  if (!params.code_challenge) {
    logOAuth('ERROR', { error: 'invalid_request', reason: 'Missing code_challenge parameter (PKCE required)' });
    return errorResponse('invalid_request', 'Missing code_challenge parameter (PKCE required)');
  }
  if (params.code_challenge_method !== 'S256') {
    logOAuth('ERROR', { error: 'invalid_request', reason: `code_challenge_method must be S256, got "${params.code_challenge_method}"` });
  ...
  if (!isValidCodeChallenge(params.code_challenge)) {
    logOAuth('ERROR', { error: 'invalid_request', reason: 'Invalid code_challenge format', code_challenge: params.code_challenge });
