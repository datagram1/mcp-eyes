 ...
 *
 * Uses C++17 filesystem and POSIX APIs.
 */
...
json FilesystemTools::search(const std::string& basePath, const std::string& globPattern, int maxResults)
{
    try
    {
        ...
        std::string fullPattern = basePath + "/" + globPattern;

        glob_t globResult;
        int flags = GLOB_TILDE | GLOB_BRACE;

        if (glob(fullPattern.c_str(), flags, nullptr, &globResult) == 0)
        {
            for (size_t i = 0; i < globResult.gl_pathc && matches.size() < static_cast<size_t>(maxResults); i++)
            {
        ...
        // If no results from glob, try recursive search with fnmatch
        if (matches.empty())
        {
